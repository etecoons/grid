...
pub async fn serve_manifest(State(state): State<AppState>) -> impl IntoResponse {
    ...
    let manifest = serde_json::json!({
        "name": title,
        "short_name": title,
        "description": "High-Performance Kanban Board",
        "start_url": "/",
        "display": "standalone",
...
pub async fn serve_asset_manifest(State(state): State<AppState>) -> impl IntoResponse {
    let manifest = state.asset_manifest.as_ref().clone();
    Json(manifest)
}
