//! CORS, HSTS, title-injection, security-headers middleware for grid.
//!
//! Per-app copy of the prior `shared_backend::middleware` modules,
//! tuned to take `Arc<crate::config::AppConfig>` instead of
//! `Arc<shared_backend::server::ServerConfig>`.
...
/// State wrapper so the app can install the CORS layer with `.layer(...)`.
#[derive(Clone)]
...
pub fn cors_layer(state: &CorsState) -> tower_http::cors::CorsLayer {
    ...
    if origins == "*" {
        ...
        CorsLayer::new().allow_origin(tower_http::cors::AllowOrigin::exact("null".parse().unwrap()))
    } else {
...
/// State wrapper so the app can install the HSTS layer with `.layer(...)`.
#[derive(Clone)]
...
/// Replace `{{SITE_TITLE}}` placeholders in HTML responses with the
...
pub async fn title_injection_layer(
    ...
    let mut new_response =
        axum::response::Response::new(axum::body::Body::from(replaced));
