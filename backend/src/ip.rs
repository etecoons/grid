...
//! Resolves the connecting client's IP from `ConnectInfo<SocketAddr>`
//! and optionally an `X-Forwarded-For` header when behind a trusted
//! proxy.
...
pub fn get_client_ip(
    ...
    // X-Forwarded-For to rotate their lockout-key IP at will. This is
    // the critical behaviour vs. the prior implementation.
